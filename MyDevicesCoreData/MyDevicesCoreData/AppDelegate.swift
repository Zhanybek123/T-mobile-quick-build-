...
//        fetchRequest()
        ...
        do {
            if let results = try context.fetch(request) as? [NSManagedObject] {
                for result in results {
                    if let deviceType = result.value(forKey: "deviceType"),
    ...
    func addTestData() {
        let context = persistentContainer.viewContext
        guard let entity = NSEntityDescription.entity(forEntityName: "Devices", in: context) else {fatalError("Couldn't find entity description")}
        for i in 1...25 {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "MyDevicesCoreData")
